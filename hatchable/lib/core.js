...
export async function generateDirections(brief) {
  ...
  return {brief:{name,sector,promise,audience,traits,must_avoid:avoid,risk_tolerance:risk},routes,image_generation_handoff:{mode:'plugin_rendering_available',instructions:'Utiliser render_brand_direction pour une route ou run_brand_workflow pour générer les trois planches dans le plugin, puis consulter get_render_job pendant l’exécution.',storage_note:`Les planches rendues par le plugin sont conservées comme actifs générés jusqu’à expiration de la rétention configurée, ${DEFAULT_ASSET_RETENTION_HOURS} heures par défaut.`},decision_rule:'Prototyper les trois routes en monochrome avant d’en choisir une. Retenir celle dont la structure — et non la finition — rend la promesse la plus facile à percevoir et la plus difficile à confondre.'};
}
...
function retentionHours(){const value=Number(process.env.GENERATED_ASSET_RETENTION_HOURS||DEFAULT_ASSET_RETENTION_HOURS);return Number.isFinite(value)&&value>0?value:DEFAULT_ASSET_RETENTION_HOURS;}
...
function imageDimensions(bytes,mimeType){if(mimeType==="image/png"&&bytes.length>24)return{width:bytes.readUInt32BE(16),height:bytes.readUInt32BE(20)};return{width:null,height:null};}
function storeAsset(job,bytes,route){const output=job.options.output_format==="jpeg"?"jpg":job.options.output_format,mime=job.options.output_format==="jpeg"?"image/jpeg":`image/${job.options.output_format}`,digest=createHash("sha256").update(bytes).digest("hex"),filename=`${safeSlug(route.route_id,"board")}-${digest.slice(0,10)}.${output}`;const dims=imageDimensions(bytes,mime),asset={asset_id:digest.slice(0,16),route_id:route.route_id,route_name:route.route_name,filename,asset_url:assetUrl(job.job_id,filename),mime_type:mime,bytes:bytes.length,sha256:digest,width:dims.width,height:dims.height};RENDER_ASSETS.set(`${job.job_id}/${filename}`,{bytes,mime_type:mime,expires_at:job.expires_at});return asset;}
...
export async function renderBrandDirection(args={}){cleanupExpired();const options=renderOptions(args),route=routePayload(args),job=newJob("render",options,{route});RENDER_JOBS.set(job.job_id,job);setTimeout(()=>{void runRenderJob(job.job_id);},0);return publicJob(job);}
export async function runBrandWorkflow(args={}){cleanupExpired();const options=renderOptions(args),directions=await generateDirections(args),job=newJob("workflow",options,{brief:directions.brief,directions});RENDER_JOBS.set(job.job_id,job);setTimeout(()=>{void runWorkflowJob(job.job_id);},0);return publicJob(job);}
